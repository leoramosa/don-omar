...
class Mouse {
  constructor(x, y) {
    ...
    window.addEventListener("mousemove", e => {
      console.log($('div.marc').width() + ' - '+ $('div.marc').height());
  ...
  resize() {
      ...
      var position =  $('img.mc_titulo').position();
      console.log('redimen ==>' + position.left + ' ' + position.top);
      console.log('redimen ==>' + this.targetPosition.x + ' ' + this.targetPosition.y);
      this.targetPosition.x = position.left;
      this.targetPosition.y = position.top;
      position =  $('img.mc_titulo').position();
      while(position.left != this.targetPosition.x || position.top != this.targetPosition.y){
        position =  $('img.mc_titulo').position();
        this.targetPosition.x = position.left;
        this.targetPosition.y = position.top;
        console.log('cambio');
...
class Canvas {
  constructor(selector) {
    ...
    (() => {
      ...
      element.style = `position: absolute; width: 100%; height: auto; display: none;`;
  ...
  resize() {
    ...
    this.dimensions.y = this.frame.height = this.element.height = window.innerHeight;
      //this.dimensions.x = $('img.mc_titulo').width();
      //this.dimensions.y = $('img.mc_titulo').height(); 
